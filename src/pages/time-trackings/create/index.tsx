...
function TimeTrackingCreatePage() {
  ...
  const formik = useFormik<TimeTrackingInterface>({
    initialValues: {
      punch_in: new Date(new Date().toDateString()),
      punch_out: new Date(new Date().toDateString()),
    ...
    >
      <Box rounded="md">
        ...
        <FormWrapper onSubmit={formik.handleSubmit}>
          <FormControl id="punch_in" mb="4">
            <FormLabel fontSize="1rem" fontWeight={600}>
              Punch In
            </FormLabel>
            <DatePicker
              selected={formik.values?.punch_in ? new Date(formik.values?.punch_in) : null}
              onChange={(value: Date) => formik.setFieldValue('punch_in', value)}
            />
          </FormControl>
          <FormControl id="punch_out" mb="4">
            <FormLabel fontSize="1rem" fontWeight={600}>
              Punch Out
            </FormLabel>
            <DatePicker
              selected={formik.values?.punch_out ? new Date(formik.values?.punch_out) : null}
              onChange={(value: Date) => formik.setFieldValue('punch_out', value)}
            />
          </FormControl>
